...
public class LeaveHistoryService {
	...
	public String approveDeny(int leaveId, int managerId, String status, String mgrComments) {
		LeaveHistory leaveHistory = leaveHistoryRepository.findById(leaveId).get();
		int leaveApplied = leaveHistory.getLeaveNoOfDays();
		int empId = leaveHistory.getEmpId();
		Employee employee = employeeRepository.findById(empId).get();
		int mgrId = employee.getEmpManagerId();
		if (mgrId != managerId) {
			return "You are UnAuthorized to Approve or Reject Leave...";
		}
		if (status.toUpperCase().equals("YES")) {
			...
			jdbcTemplate.update(cmd, new Object[] {mgrComments, leaveId});
			return "Your Leave Approved Successfully...Enjoy";
		} else {
			...
			cmd = "Update Employee set EMP_AVAIL_LEAVE_BAL = EMP_AVAIL_LEAVE_BAL + ? WHERE "
					+ " EMP_ID = ?";
			jdbcTemplate.update(cmd, new Object[] {leaveApplied, empId});
			return "Leave Rejected and Leave Balance Updated...";
			
		}
	}
	
	public List<LeaveHistory> showEmployeeLeaveHistory(int empId) {
		return leaveHistoryRepository.findByEmpId(empId);
	}
	
	public LeaveHistory searchByLeaveId(int leaveId) {
		return leaveHistoryRepository.findByLeaveId(leaveId);
	}
